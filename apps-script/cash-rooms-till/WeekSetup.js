...
function getDayData(cfg, weekSheetName, row) {
  ...
  // Column mappings for daily data
  return {
    cash: sheet.getRange(row, 4).getValue() || 0,           // D: Cash in till (manager enters)
    cashLessFloat: sheet.getRange(row, 5).getValue() || 0,  // E: Cash less float (formula)
    cashToSafe: sheet.getRange(row, 6).getValue() || 0,     // F: Suggested cash to safe (formula)
    actualToSafe: sheet.getRange(row, 7).getValue() || 0,   // G: Actual to safe (manager enters)
    dailyOutNotes: sheet.getRange(row, 8).getValue() || '', // H: Daily Out Notes / Cash Diff Notes
    pdq1: sheet.getRange(row, 9).getValue() || 0,           // I: PDQ 1
    pdq2: sheet.getRange(row, 10).getValue() || 0,          // J: PDQ 2
    grossOnTill: sheet.getRange(row, 12).getValue() || 0,   // L: Gross on Till
    netOnTill: sheet.getRange(row, 13).getValue() || 0,     // M: Net on Till
    itemVoid: sheet.getRange(row, 14).getValue() || 0,      // N: Item Void
    tabs: sheet.getRange(row, 15).getValue() || 0,          // O: Tabs
    salesNotes: sheet.getRange(row, 18).getValue() || '',   // R: Sales Notes
    pdqRooms: sheet.getRange(row, 19).getValue() || 0,      // S: Rooms Card (UPDATED from T)
    cashRooms: sheet.getRange(row, 21).getValue() || 0,     // U: Room Cash (UPDATED from V)
    receipts: _getDayReceipts(sheet, row)                   // Daily receipts (cash from till), from AB1 store
  };
}
...
function _applyDailyReceiptsRollup(cfg, sheet) {
  ...
  for (var dayKey in store) {
    ...
    for (var i = 0; i < items.length; i++) {
      ...
      descs[cat].push((desc || 'Receipt') + ' \u00a3' + amt.toFixed(2));
      dayTotal += amt;
    }
...
function getPreviousDayTabBalance(cfg, weekSheetName, dayRow) {
  ...
  for (let row = 7; row < dayRow; row++) {
    const dayTabMovement = weekSheet.getRange(row, 15).getValue() || 0; // Column O = Tabs
    balance += dayTabMovement;
  }
  return balance;
}
...
function getWeeklyFormData(cfg, weekSheetName) {
  ...
  // Fast path for web app: openById + light week finder. Do NOT call
  // findMostRecentDraftWeek / getRunningBalances here — those walk every
  // sheet and were freezing Weekly Cash Up on manager.eightbellsbolney.com.
  const weekSheet = _findWeekSheetForForm(cfg, weekSheetName);
  ...
  // Batch-read daily totals row (D14:U14) — one round-trip instead of 11
  const row14 = weekSheet.getRange(14, 4, 1, 18).getValues()[0]; // D=0 … U=17
  const dailyCash = row14[0] || 0;
  const cashLessFloat = row14[1] || 0;
  const cashToSafe = row14[2] || 0;
  const actualCashToSafe = row14[3] || 0;
  const cardTotal = row14[7] || 0;       // K
  const grossOnTill = row14[8] || 0;     // L
  const netOnTill = row14[9] || 0;       // M
  const tabs = row14[11] || 0;           // O
  const dailyOut = row14[13] || 0;       // Q
  const roomsCard = row14[15] || 0;      // S
  const roomCash = row14[17] || 0;       // U
  ...
  // Batch-read expense amounts + descriptions (D24:E41)
  const expenseBlock = weekSheet.getRange(24, 4, 18, 2).getValues();
  let totalExpenses = 0;
  let expenses = {};
  ...
  for (let i = 0; i < 18; i++) {
    const row = 24 + i;
    const amount = expenseBlock[i][0] || 0;
    const descriptionsCell = expenseBlock[i][1] || '';
    ...
    if (amount !== 0) {
      ...
      expenses[row] = [];
      if (descriptionsCell) {
        const descParts = String(descriptionsCell).split(', ');
        for (let j = 0; j < descParts.length; j++) {
          ...
          if (match) {
            ...
            expenses[row].push({
              ...
              amount: amount / descParts.length
            });
          }
        }
      } else {
        expenses[row].push({ description: 'Expense', amount: amount });
      }
    }
  }

  const weeklyBlock = weekSheet.getRange(45, 4, 11, 1).getValues(); // D45:D55
  const actualDailyCashAfterExpenses = weeklyBlock[0][0] || 0;
  const cashToCharlie = weeklyBlock[1][0] || 0;
  const differenceCashToCharlie = weeklyBlock[2][0] || 0;
  const dailyCashShort = weeklyBlock[4][0] || 0;
  const weeklyDifferenceSales = weeklyBlock[6][0] || 0;
  const tabToNetDiff = weeklyBlock[8][0] || 0;
  ...
  // Skip tab walker — weekly form UI does not use these; menu/daily still do.
  return {
    weekSheet: weekSheet.getName(),
    status: weekStatus,
    weekEnding: Utilities.formatDate(weekEndingDate, Session.getScriptTimeZone(), 'dd/MM/yyyy'),
    ...
    runningTabBalance: tabs,
    runningCashDiff: 0,

    weeklyNotes: weekNotes,
    expenseDetails: expenses,
    dailyReceipts: (typeof _getWeekDailyReceipts === 'function')
      ? _getWeekDailyReceipts(weekSheet)
      : []
  };
}
...
function getExpenseCategories(cfg) {
  ...
  for (let row = 24; row <= 41; row++) {
    const name = templateSheet.getRange(row, 1).getValue() || '';
    if (name) {
      let icon = '💰';
